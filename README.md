<div align="center">

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=22&pause=1000&color=F1F1F1&center=true&vCenter=true&random=false&width=650&lines=%E2%8A%B9+Welcome+to+my+profile!+%CB%99%E1%B5%95%CB%99+%E2%8A%B9+;Gustavo+Henrique+%7C+20+anos+%F0%9F%90%AC;Estudante+de+Engenharia+de+Software" alt="Typing SVG">

</div>

#

<div align="center">

### 👨‍💻 Gustavo Henrique

**Estudante de Engenharia de Software • Desenvolvedor em formação**

🐍 Python &nbsp;•&nbsp; ☕ Java &nbsp;•&nbsp; 🗄️ SQL &nbsp;•&nbsp; 📱 Flutter

<br>

<a href="https://github.com/Devgusta-py/Devgusta-py">
  <img src="https://img.shields.io/badge/Ver%20meu%20GitHub-00AEEF?style=for-the-badge&logo=github&logoColor=white" alt="GitHub">
</a>

</div>

#

<img align="right" alt="Cyberpunk" height="190px" src="./src/edgerunners.gif">

## 👋 Sobre mim

Me chamo **Gustavo Henrique**, tenho **20 anos** e sou estudante de **Engenharia de Software**.

Sou apaixonado por tecnologia e desenvolvimento de software. Atualmente, meu foco principal está em **Python** e **Java**, buscando aprimorar minha lógica de programação, orientação a objetos e desenvolvimento de aplicações.

Também estudo **SQL** para trabalhar com bancos de dados e **Flutter** para desenvolvimento de aplicações multiplataforma.

Gosto de aprender através da prática, criar projetos e transformar novos conhecimentos em soluções reais.

<br clear="right"/>

#

## 🧰 Minha Stack

<div align="center">

<img 
    src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/python/python-original.svg"
    width="55px"
    alt="Python"
    title="Python"
/>

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

<img 
    src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/java/java-original.svg"
    width="55px"
    alt="Java"
    title="Java"
/>

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

<img 
    src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/mysql/mysql-original.svg"
    width="55px"
    alt="SQL"
    title="SQL"
/>

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

<img 
    src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/flutter/flutter-original.svg"
    width="55px"
    alt="Flutter"
    title="Flutter"
/>

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

<img 
    src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/git/git-original.svg"
    width="55px"
    alt="Git"
    title="Git"
/>

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;

<img 
    src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/github/github-original.svg"
    width="55px"
    alt="GitHub"
    title="GitHub"
/>

</div>

<div align="center">

**Python** &nbsp;&nbsp;&nbsp; **Java** &nbsp;&nbsp;&nbsp; **SQL** &nbsp;&nbsp;&nbsp; **Flutter** &nbsp;&nbsp;&nbsp; **Git** &nbsp;&nbsp;&nbsp; **GitHub**

</div>

#

## 🚀 Em foco

<div align="center">

| Tecnologia | Objetivo |
|:---:|:---|
| 🐍 **Python** | Desenvolvimento e prática de programação |
| ☕ **Java** | Programação Orientada a Objetos |
| 🗄️ **SQL** | Banco de dados e consultas |
| 📱 **Flutter** | Desenvolvimento de aplicações |
| 🐙 **Git & GitHub** | Versionamento e projetos |

</div>

#

## 📊 GitHub Stats

<div align="center">

<img 
    height="180"
    src="https://github-readme-stats-two-omega-43.vercel.app/api?username=Devgusta-py&show_icons=true&hide_border=true&bg_color=0D1117&title_color=00AEEF&text_color=FFFFFF&icon_color=00AEEF&locale=pt-br&custom_title=Estatísticas+do+GitHub"
    alt="Estatísticas do GitHub"
/>

&nbsp;&nbsp;

<img 
    height="180"
    src="https://github-readme-stats-two-omega-43.vercel.app/api/top-langs/?username=Devgusta-py&layout=compact&hide_border=true&bg_color=0D1117&title_color=00AEEF&text_color=FFFFFF&locale=pt-br&custom_title=Principais+Linguagens"
    alt="Principais linguagens"
/>

</div>

#

## 📌 Atualmente

🎓 Estudando **Engenharia de Software**  
🐍 Aprofundando meus conhecimentos em **Python**  
☕ Desenvolvendo minhas habilidades com **Java**  
🗄️ Estudando **SQL e bancos de dados**  
📱 Explorando **Flutter**  
🚀 Criando projetos para evoluir na prática

#

<div align="center">

### 🐬 Sempre aprendendo, sempre evoluindo.

<br>

<a href="https://github.com/Devgusta-py">
<img src="https://img.shields.io/badge/Devgusta--py-GitHub-00AEEF?style=for-the-badge&logo=github&logoColor=white">
</a>

</div>
